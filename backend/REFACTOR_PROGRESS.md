...
### Phase 3 â€” Replace giant prompts
- Added `backend/prompts/`:
  - `PromptBuilder.ts` â€” shared `preamble()` + `renderContext(LLMContext)`. The render function emits a compact, stable text block (board + components + protocol buckets + relevant examples) so each stage prompt only needs to add the stage-specific instruction.
  - `plan.prompt.ts` â€” Phase 4 planner prompt. ~25 lines.
  - `components.prompt.ts` â€” Phase 5 component-instance prompt.
  - `connections.prompt.ts` â€” Phase 5 wiring prompt.
  - `libraries.prompt.ts` â€” Phase 7 library prompt.
  - `firmware.prompt.ts` â€” Phase 9 firmware prompt.
...
- **Phase 4** â€” Add `planner/Planner.ts` and `planner/types.ts` (`ProjectPlan` interface). Use `buildPlanPrompt` to ask the LLM for a plan.
- **Phase 5** â€” Rewrite `api/generate.ts` pipeline (Planner â†’ Components â†’ Connections â†’ Libraries â†’ Firmware â†’ Validation) and migrate the remaining legacy call sites to `CatalogService`. Remove the old `systemPrompt` blobs.
- **Phase 6** â€” Cap retry loop to one repair attempt.
- **Phase 7** â€” Extract `LibraryResolver.ts` (drives `libraries.prompt.ts`).
- **Phase 8** â€” Add `PinAllocator.ts` (drives `firmware.prompt.ts`).
- **Phase 9** â€” Wire up `firmware.prompt.ts` into the new pipeline.
- **Phase 10** â€” Dead code, commented prompts, Groq leftovers, add unit tests.
- **Phase 11** â€” Maintain this file.
...
Phase 3 finished. The new prompts are tiny and context-driven. Next concrete action: **Phase 4 â€” Planner**.
